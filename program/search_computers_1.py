...
def searchComputersLocally(serverHostname, hostname, ip, mac):
    ...
    with open(join("../data/input/simplified-data", serverHostname.lower() + ".csv"), mode = "r", encoding = "big5", newline = "") as computerFile:
        ...
        for computer in reader:
            ...
            isHostnameMatch = hostname is None or computer_hostname.lower() == hostname.lower()
            isIpMatch = ip is None or computer_ip == ip
            isMacMatch = mac is None or computer_mac.lower() == mac.lower()
...
def writeSearchResults(searchResults):
    with open(join("../data/output", "search-results-1.csv"), mode = "w", encoding = "big5", newline = "") as computerFile:
